...
public class ComponentProvider {
    ...
    private final Project project; // 新增：用于智能过滤
    ...
    public ComponentProvider(Project project) {
        ...
        this.project = project; // 初始化 project
    ...
    private void applySmartFiltering() {
        try {
            ...
            if (filteredOutCount > 0) {
                VueKitLogger.info(LOG, "  过滤原因: 根据项目配置和用户设置，隐藏了不相关的组件库");
            }

        } catch (Exception e) {
            VueKitLogger.error(LOG, "应用智能过滤失败", e);
            // 过滤失败时保持原始组件列表，确保功能可用
        }
    }
...
//            String adjustedDescription = adjustComponentDescription(componentInfo.getDescription());
     ...
     * 判断逻辑：
     * 1. 首先检查是否是自定义组件库的组件
     * 2. 如果是自定义组件，返回其所属的自定义组件库版本
     * 3. 如果不是自定义组件，尝试从已安装的组件库中获取版本
     * 4. 如果无法获取版本，返回默认版本信息
     *
     * @param componentName 要查询的组件名称，不能为 null 或空字符串
     * @return 组件所属的组件库版本
    ...
    public String getComponentLibraryVersion(String componentName) {
        ...
        VueKitLogger.debug(LOG, "查询组件 '" + componentName + "' 所属的组件库版本");

        // 检查是否是自定义组件库的组件
        if (CustomComponentLibraryManager.isCustomComponent(componentName)) {
            String customLibraryVersion = CustomComponentLibraryManager.getCustomLibraryVersion(componentName);
            VueKitLogger.debug(LOG, "组件 '" + componentName + "' 属于自定义组件库，版本: " + customLibraryVersion);
            return customLibraryVersion;
        }
        ...
        try {
            ...
            for (ComponentLibrary library : installedLibraries) {
                if (library.getName() != null) {
                    // 使用 packageName 进行比较，因为 ComponentLibrary 的 name 字段通常是包名格式
                    if (library.getName().equalsIgnoreCase(libraryType.getPackageName()) ||
                        library.getName().equalsIgnoreCase(libraryType.getDisplayName())) {
                        String version = library.getVersion();
                        if (version != null && !version.trim().isEmpty()) {
                            VueKitLogger.debug(LOG, "组件 '" + componentName + "' 属于内置组件库，版本: " + version);
                            return version;
                        }
                    }
                }
        ...
        // 如果无法获取版本，返回默认版本信息
        String defaultVersion = getDefaultLibraryVersion();
        VueKitLogger.debug(LOG, "组件 '" + componentName + "' 使用默认版本: " + defaultVersion);
